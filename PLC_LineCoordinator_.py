...
# >>> WEEK 2 DELIVERABLE: REMOVE HARDCODED BUF_MAX <<<
# BUF_MAX = 2  # REMOVED - now loaded from line_config.json
...
class PLC_LineCoordinator:
    def __init__(self, args):
        ...
        # >>> WEEK 2 DELIVERABLE: Parameterized buffer capacities <<<
        self._buffer_capacity = self._load_buffer_config()  # Load from line_config.json
        print(f"PLC: Loaded buffer capacities: {self._buffer_capacity}")
        
        # pipeline buffers (initialized in reset)
        self._buffers = {
            "S1_to_S2": 0,
            "S2_to_S3": 0,
            "S3_to_S4": 0,
    ...
    # >>> WEEK 2 DELIVERABLE: Buffer config loader <<<
    def _load_buffer_config(self) -> dict:
        """
        Load buffer capacities from line_config.json.
        Returns dict with keys: S1_to_S2, S2_to_S3, S3_to_S4, S4_to_S5, S5_to_S6
        """
        ...
        if os.path.exists(config_path):
            try:
                with open(config_path, 'r') as f:
                    ...
                    if "buffers" in cfg:
                        loaded = cfg["buffers"]
                        # Validate all required buffers exist
                        validated = {}
                        for buf_key in default_buffers.keys():
                            if buf_key in loaded:
                                cap = int(loaded[buf_key])
                                if cap < 0:
                                    print(f"  ⚠️  PLC: Buffer '{buf_key}' capacity {cap} < 0 - using default {default_buffers[buf_key]}")
                                    validated[buf_key] = default_buffers[buf_key]
                                else:
                                    validated[buf_key] = cap
                            else:
                                print(f"  ⚠️  PLC: Missing buffer '{buf_key}' in config - using default {default_buffers[buf_key]}")
                                validated[buf_key] = default_buffers[buf_key]
                        print(f"  ✅ PLC: Loaded buffer capacities from {config_path}")
                        return validated
                    else:
                        print(f"  ⚠️  PLC: line_config.json missing 'buffers' section - using defaults")
            except Exception as e:
                print(f"  ⚠️  PLC: Error loading {config_path}: {e} - using defaults")
        else:
            print(f"  ⚠️  PLC: {config_path} not found - using default buffer capacities")
            # Create default config file for user convenience (only if ST1 hasn't created it yet)
    ...
    def _create_default_config(self):
        """Create default line_config.json with buffer capacities (user convenience)"""
        default_full_config = {
        ...
        try:
            ...
            print("  ✅ PLC: Created default line_config.json with buffer capacities")
        except Exception as e:
            print(f"  ⚠️  PLC: Could not create default config: {e}")
    ...
    def mainThread(self):
        ...
        try:
            ...
            # Virtual buffers (tokens) between stations - REINITIALIZED WITH CONFIG VALUES
            self._buffers = {
                "S1_to_S2": 0,
                "S2_to_S3": 0,
                "S3_to_S4": 0,
            ...
            print(f"PLC: Buffer capacities: {self._buffer_capacity}")
            # End of user custom code region.
            ...
            while(vsiCommonPythonApi.getSimulationTimeInNs() < self.totalSimulationTime):
                ...
                print(f"Buffers: S1->S2={self._buffers['S1_to_S2']}/{self._buffer_capacity['S1_to_S2']}, "
                      f"S2->S3={self._buffers['S2_to_S3']}/{self._buffer_capacity['S2_to_S3']}, "
                      f"S3->S4={self._buffers['S3_to_S4']}/{self._buffer_capacity['S3_to_S4']}, "
                      f"S4->S5={self._buffers['S4_to_S5']}/{self._buffer_capacity['S4_to_S5']}, "
                      f"S5->S6={self._buffers['S5_to_S6']}/{self._buffer_capacity['S5_to_S6']}")
                ...
                # ---- WAIT_S1_DONE: Wait for S1 to complete ----
                    ...
                    if completion_condition and s1_ready and not s1_busy and not s1_fault:
                        ...
                        self._s1_wait_counter = 0
                        # >>> WEEK 2 DELIVERABLE: Use config-driven buffer capacity <<<
                        max_cap = self._buffer_capacity["S1_to_S2"]
                        self._buffers["S1_to_S2"] = min(self._buffers["S1_to_S2"] + 1, max_cap)
                        print(f"PLC: Incremented S1_to_S2 buffer to {self._buffers['S1_to_S2']}/{max_cap}")
                        ...
                        max_cap = self._buffer_capacity["S1_to_S2"]
                        self._buffers["S1_to_S2"] = min(self._buffers["S1_to_S2"] + 1, max_cap)
                        print(f"PLC: Forced S1_to_S2 buffer to {self._buffers['S1_to_S2']}/{max_cap}")
                    else:
                ...
                # ---- START_S2: Send start pulse to S2 ----
                    ...
                    if s2_busy and not self._start_sent["S2"]:
                        ...
                        if not self._start_sent["S2"]:
                            ...
                            self._state = "WAIT_S2_DONE"
                            # >>> WEEK 2 DELIVERABLE: Use config-driven buffer capacity <<<
                            # Consume buffer BEFORE starting station
                            self._buffers["S1_to_S2"] = max(0, self._buffers["S1_to_S2"] - 1)
                            print(f"PLC: Consumed S1_to_S2 buffer, now {self._buffers['S1_to_S2']}/{self._buffer_capacity['S1_to_S2']}")
                    else:
                ...
                # ---- WAIT_S2_DONE: Wait for S2 to complete ----
                    ...
                    if completion_condition and s2_ready and not s2_busy and not s2_fault:
                        ...
                        self._s2_wait_counter = 0
                        # >>> WEEK 2 DELIVERABLE: Use config-driven buffer capacity <<<
                        max_cap = self._buffer_capacity["S2_to_S3"]
                        self._buffers["S2_to_S3"] = min(self._buffers["S2_to_S3"] + 1, max_cap)
                        print(f"PLC: Incremented S2_to_S3 buffer to {self._buffers['S2_to_S3']}/{max_cap}")
                        ...
                        max_cap = self._buffer_capacity["S2_to_S3"]
                        self._buffers["S2_to_S3"] = min(self._buffers["S2_to_S3"] + 1, max_cap)
                        print(f"PLC: Forced S2_to_S3 buffer to {self._buffers['S2_to_S3']}/{max_cap}")
                    else:
                ...
                # ---- START_S3: Send start pulse to S3 ----
                    ...
                    if s3_busy and not self._start_sent["S3"]:
                        ...
                        if not self._start_sent["S3"]:
                            ...
                            print(f"PLC: Consumed S2_to_S3 buffer, now {self._buffers['S2_to_S3']}/{self._buffer_capacity['S2_to_S3']}")
                    else:
                ...
                # ---- WAIT_S3_DONE: Wait for S3 to complete ----
                    ...
                    if completion_condition and s3_ready and not s3_busy and not s3_fault:
                        ...
                        self._s3_wait_counter = 0
                        # >>> WEEK 2 DELIVERABLE: Use config-driven buffer capacity <<<
                        max_cap = self._buffer_capacity["S3_to_S4"]
                        self._buffers["S3_to_S4"] = min(self._buffers["S3_to_S4"] + 1, max_cap)
                        print(f"PLC: Incremented S3_to_S4 buffer to {self._buffers['S3_to_S4']}/{max_cap}")
                        ...
                        max_cap = self._buffer_capacity["S3_to_S4"]
                        self._buffers["S3_to_S4"] = min(self._buffers["S3_to_S4"] + 1, max_cap)
                        print(f"PLC: Forced S3_to_S4 buffer to {self._buffers['S3_to_S4']}/{max_cap}")
                    else:
                ...
                # ---- START_S4: Send start pulse to S4 ----
                    ...
                    if s4_busy and not self._start_sent["S4"]:
                        ...
                        if not self._start_sent["S4"]:
                            ...
                            print(f"PLC: Consumed S3_to_S4 buffer, now {self._buffers['S3_to_S4']}/{self._buffer_capacity['S3_to_S4']}")
                    else:
                ...
                # ---- WAIT_S4_DONE: Wait for S4 to complete ----
                    ...
                    if completion_condition and s4_ready and not s4_busy and not s4_fault:
                        ...
                        self._s4_wait_counter = 0
                        # >>> WEEK 2 DELIVERABLE: Use config-driven buffer capacity <<<
                        max_cap = self._buffer_capacity["S4_to_S5"]
                        self._buffers["S4_to_S5"] = min(self._buffers["S4_to_S5"] + 1, max_cap)
                        print(f"PLC: Incremented S4_to_S5 buffer to {self._buffers['S4_to_S5']}/{max_cap}")
                        ...
                        max_cap = self._buffer_capacity["S4_to_S5"]
                        self._buffers["S4_to_S5"] = min(self._buffers["S4_to_S5"] + 1, max_cap)
                        print(f"PLC: Forced S4_to_S5 buffer increment, now at {self._buffers['S4_to_S5']}/{max_cap}")
                    else:
                ...
                # ---- START_S5: Send start pulse to S5 ----
                    ...
                    if s5_busy and not self._start_sent["S5"]:
                        ...
                        print(f"PLC: No work: S4_to_S5 buffer empty ({self._buffers['S4_to_S5']}/{self._buffer_capacity['S4_to_S5']})")
                        ...
                        if not self._start_sent["S5"]:
                            ...
                            print(f"PLC: Consumed S4_to_S5 buffer, now {self._buffers['S4_to_S5']}/{self._buffer_capacity['S4_to_S5']}")
                    else:
                        print(f"PLC: S5 not ready to start (buffer={self._buffers['S4_to_S5']}/{self._buffer_capacity['S4_to_S5']}, ready={s5_ready}, busy={s5_busy}, fault={s5_fault})")
                # ---- WAIT_S5_DONE: Wait for S5 to complete ----
                    ...
                    if completion_condition and s5_ready and not s5_busy and not s5_fault:
                        ...
                        self._s5_wait_counter = 0
                        # >>> WEEK 2 DELIVERABLE: Use config-driven buffer capacity <<<
                        max_cap = self._buffer_capacity["S5_to_S6"]
                        self._buffers["S5_to_S6"] = min(self._buffers["S5_to_S6"] + 1, max_cap)
                        print(f"PLC: Incremented S5_to_S6 buffer to {self._buffers['S5_to_S6']}/{max_cap}")
                        ...
                        max_cap = self._buffer_capacity["S5_to_S6"]
                        self._buffers["S5_to_S6"] = min(self._buffers["S5_to_S6"] + 1, max_cap)
                        print(f"PLC: Forced S5_to_S6 buffer to {self._buffers['S5_to_S6']}/{max_cap}")
                    else:
                ...
                # ---- START_S6: Send start pulse to S6 ----
                    ...
                    if s6_busy and not self._start_sent["S6"]:
                        ...
                        if not self._start_sent["S6"]:
                            ...
                            print(f"PLC: Consumed S5_to_S6 buffer, now {self._buffers['S5_to_S6']}/{self._buffer_capacity['S5_to_S6']}")
                    else:
            ...
            # >>> WEEK 2 DELIVERABLE: Export PLC KPIs <<<
            # Generate PLC-level KPI report at simulation end
            total_sim_time_s = vsiCommonPythonApi.getSimulationTimeInNs() / 1e9
            plc_kpis = {
                "simulation_duration_s": total_sim_time_s,
                "finished_products": self.finished,
                "throughput_units_per_hour": (self.finished / total_sim_time_s) * 3600,
                "buffer_capacities": self._buffer_capacity,
                "final_buffer_levels": self._buffers.copy(),
                "s5_accept_total": self._s5_accept_total,
                "s5_reject_total": self._s5_reject_total,
                "s5_yield_rate_pct": (self._s5_accept_total / max(1, self._s5_accept_total + self._s5_reject_total)) * 100.0
            }
            
            # Write PLC KPI file
            kpi_file = f"PLC_kpis_{int(vsiCommonPythonApi.getSimulationTimeInNs()/1e9)}.json"
            with open(kpi_file, 'w') as f:
                json.dump(plc_kpis, f, indent=2)
            print(f"\n✅ PLC KPIs exported to {kpi_file}")
            print(f"   Throughput: {plc_kpis['throughput_units_per_hour']:.1f} units/hour")
            print(f"   Finished products: {plc_kpis['finished_products']}")
            print(f"   S5 Yield rate: {plc_kpis['s5_yield_rate_pct']:.1f}%")
